...
interface DeckType {
  id: string;
  name: string;
  cardCount?: number; // ahora opcional
}
...
const FlashcardManager: React.FC<FlashcardManagerProps> = ({ deckId: initialDeckId, onCreateCard, currentUserId, onCreateDeck }) => {
  ...
  const handleCreateCard = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // --- Asegurarse que la tarjeta tenga deckId correcto ---
      const formattedCard = {
        ...card,
        deckId: card.deckId || card.deck_id, // deck_id de DB -> deckId
      };

      onCreateCard(formattedCard);
      setFront("");
      setBack("");
  ...
  const handleCreateDeck = async () => {
    ...
    try {
      ...
      setDecks(prev => [...prev, { ...deck, cardCount: 0 }]);
      setDeckId(deck.id);
      setNewDeckName("");
